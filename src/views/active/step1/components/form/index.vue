<template>
  ...
  >
    ...
    <template slot="header">
      <el-button v-show="step === 1" size="mini" type="primary" :loading="btnLoading" @click="save">保存草稿</el-button>
...
import { add, edit, details, uploadFile, uploadList, start, getNextStep, getFirstStepTask, complete } from '../../../api'
...
export default {
  ...
  methods: {
    ...
    // 第一步的提交
    async doStep1 () {
      if (this.formData.drawerMode === 'add') {
        const res = await add(this.formData)
        this.formData.id = res.result
      } else if (this.formData.drawerMode === 'edit') {
        await edit(this.formData)
      }
      // 如果是草稿状态，没有启动过，则调用启动流程接口
      if (!this.formData.taskId) {
        await start({ id: this.formData.id })
      }
      this.btnLoading = false
      this.step = 2
    ...
    getTaskNextStepOption () {
      if (this.formData.taskId) {
        // 如果是已经发起过的流程，则直接调用getNextStep接口
        ...
        // 通过调用getFirstStepTask接口，拿到taskId和processInstanceId，根据taskId和instanceId查询出当前流程节点下可供选择的"流程下一步环节"备选项数据
        getFirstStepTask({ id: this.formData.id }).then(res => {
          this.formData.taskId = res.result.taskId
          this.formData.processInstanceId = res.result.processInstanceId
          getNextStep({ taskId: this.formData.taskId, processInstanceId: this.formData.processInstanceId }).then(result => {
            this.activeNextStepOption = result.result
          })
        })
