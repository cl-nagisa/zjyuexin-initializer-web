<template>
  ...
  >
    ...
    <template slot="header">
      ...
      <el-button v-show="step === 1" size="mini" type="primary" :loading="btnLoading" @click="back">回退</el-button>
    ...
    <template>
      ...
      <div class="form-wrapper">
        ...
        >
          ...
          <!-- 根据实际业务情况，选择添加任务判断字段，下面的condition字段只是个列子，实际可能会有多个 -->
          <el-form-item label="是否发送给HR" prop="condition">
        ...
        <el-form
          ...
          label-position="top"
        >
          <el-form-item label="回退环节" prop="stepIndex">
            <el-select v-model="formData3.stepIndex">
              <el-option v-for="item in rollBackStepOption" :key="item.index" :label="item.step" :value="item.index"></el-option>
            </el-select>
          </el-form-item>
          <!-- 根据实际业务需要，选择是否展现"审批批注"字段 -->
          <el-form-item label="审批批注" prop="comment">
            <el-input v-model="formData3.comment" type="textarea" :rows="4"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </template>
...
import { edit, details, uploadFile, uploadList, getNextStep, complete, updateFlow, getRollbackSteps, rollback } from '../../../api'
...
export default {
  ...
  data () {
    return {
      ...
      formData3: {},
      formRules3: {
        stepIndex: [{ required: true, message: '必填', trigger: 'change' }],
  ...
  methods: {
    btnEdit (data) {
      ...
      this.formData3 = deepcopy(defaultFormData3)
    ...
    back () {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          // 再调用updateFlow保存修改后的流程表单数据
          await updateFlow(this.formData)
          this.btnLoading = false
          // 上面两步接口调用成功后，切换表单
          this.step = 3
          // 上面两步接口调用成功后，再调用getRollbackSteps接口，获取"获取流程回退环节"备选项数据
          this.getRollbackStepOption()
        }
      })
    },
    // 获取流程回退环节
    getRollbackStepOption () {
      getRollbackSteps({ processInstanceId: this.formData.processInstanceId }).then(res => {
        this.rollBackStepOption = res.result
      })
    },
    // 下一步
    next () {
    ...
    doStep3 () {
      ...
      // 通过rollback接口回退流程到指定节点
      rollback(formData).then(() => {
        this.$notify({
          title: '提示',
          message: '提交成功',
          type: 'success'
