      ...
      <el-tab-pane label="常规" name="basic">
        ...
        <el-form :model="formData" size="mini" label-position="top" label-suffix="：">
          ...
          <el-form-item label="表达式">
            <el-input v-model="formData.conditionExpression" @change="handleChange_condition"></el-input>
          </el-form-item>
        </el-form>
        <!-- 常规end -->
      </el-tab-pane>
...
export default {
  ...
  methods: {
    // conditionExpression专用监听
    handleChange_condition (val) {
      if (this.initFlag) {
        return false
      }
      clearTimeout(this.debounceTimer)
      this.debounceTimer = setTimeout(() => {
        this.$emit('updateCondition', this.element, val)
      }, 500)
    }
  }
}
